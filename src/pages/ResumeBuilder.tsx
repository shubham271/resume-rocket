...
import {
  Sparkles, Plus, Trash2, Loader2, ChevronRight, ChevronLeft, GraduationCap, Briefcase, Award, Wrench, User,
} from "lucide-react";
...
const STEPS = ["Personal Info", "Skills", "Experience", "Education", "Certifications", "Generate"];
...
const ResumeBuilderPage = () => {
  ...
  const addExperience = () => {
    setExperiences([...experiences, { company: "", title: "", startDate: "", endDate: "", duties: "" }]);
  };
  ...
  const addEducation = () => {
    setEducations([...educations, { institution: "", degree: "", field: "", startDate: "", endDate: "" }]);
  };

  const updateEducation = (idx: number, field: keyof Education, value: string) => {
    setEducations(educations.map((edu, i) => i === idx ? { ...edu, [field]: value } : edu));
  };
    ...
    <div className="p-6 md:p-10">
      ...
      <div className="rounded-2xl border bg-card p-6">
        ...
        {step === 0 && (
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              ...
              <div className="space-y-2">
                <Label>Email</Label>
                <Input value={email} onChange={(e) => setEmail(e.target.value)} placeholder="john@example.com" className="rounded-xl" />
              </div>
              <div className="space-y-2">
                <Label>Phone</Label>
                <Input value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="+1 (555) 123-4567" className="rounded-xl" />
              </div>
              <div className="space-y-2">
                <Label>Location</Label>
                <Input value={location} onChange={(e) => setLocation(e.target.value)} placeholder="San Francisco, CA" className="rounded-xl" />
              </div>
              <div className="space-y-2">
                <Label>LinkedIn URL</Label>
                <Input value={linkedIn} onChange={(e) => setLinkedIn(e.target.value)} placeholder="https://linkedin.com/in/johndoe" className="rounded-xl" />
              </div>
            </div>
            <div className="space-y-2">
              <Label>Professional Summary</Label>
              <Textarea value={summary} onChange={(e) => setSummary(e.target.value)} placeholder="Brief summary of your professional background..." className="min-h-[80px] rounded-xl" />
            </div>
          </div>
        )}
        ...
        {step === 1 && (
          <div className="space-y-4">
            ...
            <div className="flex gap-2">
              <Input value={newSkill} onChange={(e) => setNewSkill(e.target.value)} onKeyDown={(e) => e.key === "Enter" && (e.preventDefault(), addSkill())} placeholder="e.g. React, Python, Project Management..." className="rounded-xl" />
              <Button onClick={addSkill} disabled={!newSkill.trim()} size="sm" className="rounded-xl gap-1">
            ...
            <div className="flex flex-wrap gap-2">
              {skills.map((skill, i) => (
                <Badge key={i} variant="secondary" className="gap-1 rounded-lg px-3 py-1.5">
                  {skill}
                  <button onClick={() => setSkills(skills.filter((_, j) => j !== i))} className="ml-1 hover:text-destructive">
        ...
        {step === 2 && (
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              <Button onClick={addExperience} variant="outline" size="sm" className="rounded-xl gap-1">
                <Plus className="h-4 w-4" /> Add
              </Button>
            </div>
            ...
            {experiences.map((exp, i) => (
              <div key={i} className="rounded-xl border p-4 space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <Button variant="ghost" size="sm" onClick={() => setExperiences(experiences.filter((_, j) => j !== i))} className="text-muted-foreground hover:text-destructive h-8 w-8 p-0">
                ...
                <div className="space-y-1.5"><Label className="text-xs">Key Duties & Achievements</Label><Textarea value={exp.duties} onChange={(e) => updateExperience(i, "duties", e.target.value)} placeholder="Describe your responsibilities and key achievements..." className="min-h-[60px] rounded-xl" /></div>
              </div>
            ))}
          </div>
        )}
        ...
        {step === 3 && (
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              <Button onClick={addEducation} variant="outline" size="sm" className="rounded-xl gap-1">
                <Plus className="h-4 w-4" /> Add
              </Button>
            </div>
            ...
            {educations.map((edu, i) => (
              <div key={i} className="rounded-xl border p-4 space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <Button variant="ghost" size="sm" onClick={() => setEducations(educations.filter((_, j) => j !== i))} className="text-muted-foreground hover:text-destructive h-8 w-8 p-0">
                    <Trash2 className="h-4 w-4" />
                  </Button>
                </div>
                <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
        ...
        {step === 4 && (
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              <Button onClick={addCertification} variant="outline" size="sm" className="rounded-xl gap-1">
                <Plus className="h-4 w-4" /> Add
              </Button>
            </div>
            ...
            {certifications.map((cert, i) => (
              <div key={i} className="rounded-xl border p-4 space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <Button variant="ghost" size="sm" onClick={() => setCertifications(certifications.filter((_, j) => j !== i))} className="text-muted-foreground hover:text-destructive h-8 w-8 p-0">
                    <Trash2 className="h-4 w-4" />
                  </Button>
                </div>
        ...
        {step === 5 && (
          <div className="space-y-4">
            <h2 className="font-display text-lg font-semibold flex items-center gap-2">
              <Sparkles className="h-5 w-5 text-primary" /> Generate Resume
            </h2>
